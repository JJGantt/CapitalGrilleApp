...
- **`app_logs`** (every interaction, API request and tool call, written by `AppLogger`) is the record of what the assistant was asked and did. Writes are fire-and-forget and errors are swallowed, so a broken write is silent — check Supabase's edge logs for `POST /rest/v1/app_logs` status codes when rows stop appearing.
