...
- **Getting to the watch app:** a Control Center button (`OpenControl`, watchOS 26) and a bare `CG` complication, both in `CapitalGrilleWatchWidgets`. The button's `OpenAppIntent` is compiled into the watch app as well (`project.yml` lists it under the watch target's sources), because the system runs an `OpenIntent` in the app it opens. The app no longer runs a workout session, so it backgrounds like any other app and has no HealthKit entitlement.
