...
- **Watch voice input is recorded audio, not Apple dictation.** `VoiceCapture` + `Recorder` (ported from StatusHub's watch app) record until 4s of silence after speech; a second press (double tap, or a tap while recording) locks it open, the next sends, press-and-hold throws it away. The clip goes to the hub's `/api/cg/transcribe` (status-hub `server.js`), the same Groq Whisper as the hub's own voice input, authenticated by this device's Anthropic key as `X-Owner-Key` — so only the owner's device can transcribe; a tester's watch gets a 401. The edge of the glass shows the state as StatusHub's does (`VoiceBorder`: amber recording, red locked, white sweep while transcribing and answering). The hub's copy of the owner-key hash must change with `AppGate.ownerKeyHash`.
