...
An upload only reaches the **internal** group ("cap", automatic distribution) by itself. The **public link** (`https://testflight.apple.com/join/AKmvPXaS`) belongs to the **external** group "capExt", and external groups never pick up new builds on their own — add the build to that group (App Store Connect → TestFlight → capExt → Builds → +, or the ASC API: `POST /v1/betaGroups/718c1d73-ec87-4762-906a-4c66ecc46c99/relationships/builds`), which also submits it to Beta App Review; later builds of an already-approved version clear in minutes. ASC app id `6775855684`; the API is reachable with the key below by signing an ES256 JWT with `openssl` (PyJWT is not installed).

The team's signing identities (Apple Distribution and an API-made Apple Development) are in the `gnarcast-build` keychain that surf-app's `tools/testflight.sh` created; `release.sh` unlocks it with its saved password (`~/.appstoreconnect/private_keys/gnarcast-build-keychain.password`) before archiving, because it locks after two hours idle and codesign then stops on a password dialog nobody knows the answer to. Profiles are cloud-managed, so the archive and export pass the API key with `-allowProvisioningUpdates` to authenticate to Apple's developer services. Without that they fall back to the Xcode account session, which expires — and then the export fails with `No signing certificate "iOS Distribution" found` after a "Your session has expired" line in the log (build 15, 2026-08-29). Re-signing in to Xcode is not needed; the key path is. Add testers under **Internal Testing** (no Apple review) or **External Testing** (24h review on first build).
...
- **Watch voice input is recorded audio, not Apple dictation.** A press (double tap, or a tap on the screen) starts `VoiceCapture`/`Recorder` (ported from StatusHub's watch app) and the next press sends; nothing ends a recording on its own, and the x at the bottom throws it away. The clip goes to the hub's `/api/cg/transcribe` (status-hub `server.js`), the same Groq Whisper as the hub's own voice input, authenticated by this device's Anthropic key as `X-Owner-Key` — so only the owner's device can transcribe; a tester's watch gets a 401. The edge of the glass shows the state as StatusHub's does (`VoiceBorder`: red while recording, white sweep while transcribing and answering). The hub's copy of the owner-key hash must change with `AppGate.ownerKeyHash`.
- **Getting to the watch app:** a Control Center button (`OpenControl`, watchOS 26) that opens it, and a bare `CG` complication that opens it straight into a recording (`capitalgrille://record` → `VoiceCapture.requestRecording()`, started by the chat page once the app is active, so the haptic and chirp are the same as a tap; a request older than 20s lapses rather than firing on a later wrist raise). Both are in `CapitalGrilleWatchWidgets`. The button's `OpenAppIntent` is compiled into the watch app as well (`project.yml` lists it under the watch target's sources), because the system runs an `OpenIntent` in the app it opens. The app no longer runs a workout session, so it backgrounds like any other app and has no HealthKit entitlement.
- **Models and thinking** (`AIModel` in `MacClient.swift`): Haiku 4.5, Sonnet 5, Opus 5.5; the owner defaults to Sonnet. `thinkingParams` keeps answers fast: Sonnet runs with thinking disabled; Opus 5.5 rejects `disabled`, so it runs at `effort: low`.
- **Gating** (`AppGate`, `ConfigModels.swift`) limits which backends/models a device may use: owner (the key whose hash matches `ownerKeyHash`) gets everything, anyone else API + Haiku, overridable per profile from the Supabase `app_config` table. `Backend.current` / `AIModel.current` quietly fall back to the default when the stored choice isn't allowed, so every app entry point must call `AppGate.apply()` at launch and again whenever the key changes — both targets do. Skip it and the device sits on the bundled restricted profile no matter what Settings shows.
- **`app_logs`** (every interaction, API request and tool call, written by `AppLogger`) is the record of what the assistant was asked and did. A watch voice question also writes `voice_*` rows (`VoiceLog`: recorded, transcribed, or where it failed) under the same `interaction_id`, so the whole path from press to answer is one query; the hub's side of the transcription is its `[cg-transcribe]` log line. Writes are fire-and-forget and errors are swallowed, so a broken write is silent — check Supabase's edge logs for `POST /rest/v1/app_logs` status codes when rows stop appearing.
- `food-menu.json` bundled in both targets so chat tools can resolve dish lookups offline. The app itself refreshes the menu from Supabase (`menu_dishes` rows + the `app_content` `seasonal_programs` blob) on launch, but the **Mac MCP reads this checkout's JSON, not Supabase** — so a menu edit made in Supabase has to land in the JSON too, or the Mac backend answers from stale data.
- **Seasonal programs** — the limited-time cards (Generous Pour, Wagyu & Wine, …), any number at once, stored as ONE `app_content` row, key `seasonal_programs`, whose `data` is a JSON array in display order, with the same array in `food-menu.json`. Each program: `meta{title, dates_active, notes[]}`, `wines[]` (name/producer/region/varietal/description/tasting_notes/suggested_pairing/image_url), `sections[{title, paired_wines[], dishes[]}]`. Card titles, section headers, and the prompt summary all come from the data, so adding a card is a data change (append to the array in both places), not a build. **A card is removed only when Jared says so** — a promotion's printed end date is not that; on 2026-08-29 Generous Pour was dropped on the strength of its end date and had to be restored. Section dishes are stubs; a stub whose `name` or `menu_name` normalizes to a regular-menu dish borrows that dish's qualitative recipe details in the detail view (never its portion, price, or calories). Chat reaches it through `get_seasonal_program` (`mcp__bottle__get_seasonal_program` on the Mac); the routing rule in the remote `base_rules` block uses `{{seasonal_tool}}` and `{{seasonal_skeleton}}` placeholders. The program's wines are deliberately NOT rows in `bottles` — that table is what the bar physically stocks.

## In-app AI assistant — two backends
`ChatEngine.swift`'s AI chat has **two backends**, chosen by `Backend.current` (`MacClient.swift`); both default to **`.api`** for the owner:
- **`.mac`** → routes through the Mac (`MacClient.ask`), which runs `claude -p` with an MCP server. The bottle tools are a Python MCP at **`~/scripts/capital_grille_wine_mcp.py`** (`mcp__bottle__*`). MCP reloads per session, so edits take effect next request.
